...
sub singletest_run {
    ...
    if($cmdtype eq "perl") {
        ...
        $CURLOUT = stdoutfn($LOGDIR, $testnum); # sends received data to stdout
    ...
    $CMDLINE .= "$cmdargs > " . stdoutfn($LOGDIR, $testnum) . " 2> " . stderrfn($LOGDIR, $testnum);
...
# Called by controller (via a more specific function)
